            ...
            <div style={{overflowX:"auto"}}>
              <table style={S.table}>
                <thead><tr style={{background:"#f1f5f9"}}>
                  {["N°","Date","Étape","Diagnostic","Observations","Versement",""].map(h=><th key={h} style={S.th}>{h}</th>)}
                </tr></thead>
                <tbody>
                  {sorted.map((s,i)=>(
...
function ActesPage({patients,actes,onNew,onDelete,onEdit,onTerminate}){
  ...
  const matchedPatIds=q ? new Set(patients.filter(p=>`${p.prenom||""} ${p.nom||""} ${patCode(p.id)}`.toLowerCase().includes(q)).map(p=>p.id)) : null;
...
function ActeModal({patients,actes,praticien,onClose,onSave}){
  ...
  const suggestions=patSearch.trim() ? patients.filter(p=>`${p.prenom||""} ${p.nom||""} ${patCode(p.id)}`.toLowerCase().includes(patSearch.toLowerCase())) : patients.slice(0,8);
  const selectPatient=(p)=>{ setPatientId(p.id); setPatSearch(`${p.prenom} ${p.nom}`); setShowSugg(false); setItems([]); };
            ...
            <div style={{position:"absolute",top:"100%",left:0,right:0,background:"#fff",border:"1px solid #e2e8f0",borderRadius:8,boxShadow:"0 8px 24px rgba(0,0,0,.12)",zIndex:200,maxHeight:200,overflowY:"auto",marginTop:2}}>
              {suggestions.map(p=>(
                <div key={p.id} style={{padding:"8px 12px",cursor:"pointer",display:"flex",alignItems:"center",gap:8,fontSize:13}}
                  ...
                  <div style={{width:30,height:30,borderRadius:"50%",background:"#1e40af",color:"#fff",display:"flex",alignItems:"center",justifyContent:"center",fontSize:11,fontWeight:700,flexShrink:0}}>{p.prenom?.[0]||"?"}{p.nom?.[0]||""}</div>
                  <div>
                    <div style={{fontWeight:600,color:"#0f172a"}}>{p.prenom} {p.nom}</div>
                ...
                {[...filtered].sort((a,b)=>b.date.localeCompare(a.date)).map(a=>{
                  ...
                  return(
                    <tr key={a.id} style={{borderBottom:"1px solid #f1f5f9"}}>
                      ...
                      <td style={S.td}>
                        <span style={{fontWeight:600,color:"#0f172a"}}>{pat?`${pat.prenom} ${pat.nom}`:"—"}</span>
