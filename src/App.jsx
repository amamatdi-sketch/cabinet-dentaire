...
export default function App() {
  ...
  if(!loggedIn) return <LoginPage onLogin={(u,p)=>{
    if(u===LOGIN_USER && p===LOGIN_PASS){
      localStorage.setItem("cab_auth","1");
      setLoggedIn(true);
  ...
  return(
    <div style={S.app}>
      ...
      {/* MODALS */}
      {modal==="newPat"   && <PatientModal onClose={close} onSave={async d=>{await db.from("patients").insert([{...d,id:uid(),dateCreation:now()}]);close();loadPatients();}}/>}
  ...
  return(
    <div style={S.loginBg}>
      <div style={S.loginCard}>
        ...
        <div style={S.fGroup}>
          <label style={S.fLabel}>👤 Nom d'utilisateur</label>
          <input style={S.fInput} value={u} onChange={e=>setU(e.target.value)} placeholder="admin" onKeyDown={e=>e.key==="Enter"&&onLogin(u,p)}/>
        </div>
        <div style={S.fGroup}>
          <label style={S.fLabel}>🔑 Mot de passe</label>
          <div style={{position:"relative"}}>
            <input style={{...S.fInput,width:"100%",boxSizing:"border-box",paddingRight:40}} type={show?"text":"password"} value={p} onChange={e=>setP(e.target.value)} placeholder="••••••••" onKeyDown={e=>e.key==="Enter"&&onLogin(u,p)}/>
            <button style={{position:"absolute",right:8,top:"50%",transform:"translateY(-50%)",background:"none",border:"none",cursor:"pointer",color:"#94a3b8",fontSize:14}} onClick={()=>setShow(!show)}>{show?"🙈":"👁️"}</button>
          </div>
        </div>
        <button style={{...S.btnBlue,width:"100%",padding:"12px",fontSize:15,marginTop:8}} onClick={()=>onLogin(u,p)}>→ Connexion</button>
        <p style={{color:"#94a3b8",fontSize:11,marginTop:16}}>Modifiez les identifiants dans GitHub (App.jsx ligne 7-8)</p>
      </div>
    </div>
  );
}
    ...
    <div>
      ...
      <div style={S.statsGrid}>
        {[
          ...
          {l:"Actes aujourd'hui", v:actesAuj.length,                   icon:"🦷",c:"#10b981"},
      ...
      <h2 style={{fontSize:15,fontWeight:700,color:"#0f172a",margin:"24px 0 10px"}}>Activité récente</h2>
      {actes.slice(0,6).map(a=>(
        <div key={a.id} style={S.recentRow}>
          <span style={{color:"#64748b",fontSize:12}}>{a.date}</span>
          <span style={{fontWeight:600,color:"#0f172a"}}>{a.patientId}</span>
          <span>{a.typeActe}{a.dents?` (D${a.dents})`:""}</span>
  ...
  return(
    <div style={S.ficheWrap}>
      ...
      {refs.map(ref=>{
        ...
        const first=sorted[0];
        return(
          <div key={ref} style={{...S.treatCard,...(termine?{opacity:0.7}:{})}}>
            ...
            <div style={{display:"flex",flexDirection:"column",gap:3,marginBottom:8}}>
              <div style={S.sessHdr}><span>N°</span><span>Date</span><span>Étape</span><span>Diagnostic</span><span>Versement</span><span>Actions</span></div>
              {sorted.map((s,i)=>(
                <div key={s.id} style={S.sessRow}>
                  <span style={{fontWeight:700,color:"#1e40af"}}>S{s.seanceNum||i+1}</span>
                  <span>{s.date}</span>
                  <span>{s.etape||"—"}</span>
                  <span style={{color:"#64748b",fontSize:11}}>{s.diagnostic||"—"}</span>
                  <span style={{color:"#16a34a",fontWeight:600}}>+{fmt(s.montantVerse||0)}</span>
                  <button style={S.btnEditSm} onClick={()=>onEditVersement(s)} title="Modifier le versement">💳</button>
                </div>
              ))}
            </div>
            ...
            {!termine&&(
              <button style={S.btnTerminer} onClick={()=>onTerminate(ref)}>✅ Marquer comme terminé</button>
            )}
          </div>
        );
      })}
...
function ActesPage({patients,actes,onNew,onDelete,onEdit,onTerminate}){
  ...
  return(
    <div>
      ...
      {Object.entries(groups).map(([ref,_])=>{
          ...
          <div key={ref} style={{...S.acteCard,...(termine?{borderLeftColor:"#94a3b8"}:{})}}>
            ...
            {sorted.map((s,i)=>(
              <div key={s.id} style={S.seanceRow}>
                ...
                <span style={{flex:1,minWidth:0,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{s.etape||"—"}</span>
...
// ══════════════════════════════════════════════════════════════════════════════
// MODAL NOUVELLE SÉANCE — multi-traitement
// ══════════════════════════════════════════════════════════════════════════════
function ActeModal({patients,actes,praticien,onClose,onSave}){
  ...
  // Liste des items de la séance: {type:'continuer'|'nouveau', ref?, typeActe?, dents?, quantite?, prixUnitaire?, remise?}
  const [items, setItems] = useState([]);

  const patActes = actes.filter(a=>a.patientId===patientId);

  // Traitements ouverts (pas terminés)
  const openGroups={};
  ...
  const addContinuer=()=>setItems(it=>[...it,{type:"continuer",ref:"",remise:0}]);
  const addNouveau=()=>setItems(it=>[...it,{type:"nouveau",typeActe:"Soin",dents:"",quantite:1,prixUnitaire:5000,remise:0}]);

  const handleSave=()=>{
    if(!patientId) return alert("Sélectionnez un patient");
    if(items.length===0) return alert("Ajoutez au moins un acte");
    const list=[];
    items.forEach((item,idx)=>{
      if(item.type==="continuer"){
        ...
        list.push({
          ...
          etape,diagnostic,observations,
          montantVerse:idx===0?Number(versement)||0:0,
    ...
    if(list.length===0) return alert("Veuillez sélectionner les traitements");
    onSave(list);
  };
  ...
  return(
    <Modal title="Nouvelle Séance Clinique" onClose={onClose} wide>
      ...
      {patientId&&(
        <div style={S.modeRow}>
          {openList.length>0&&<button style={S.modeBtn} onClick={addContinuer}>🔄 Continuer un traitement</button>}
          <button style={{...S.modeBtn,...S.modeBtnOn}} onClick={addNouveau}>✨ Nouveau traitement</button>
        </div>
      )}
      ...
      {items.length>0&&(
        <div style={{marginBottom:12}}>
          <div style={{fontSize:12,fontWeight:700,color:"#374151",marginBottom:6,textTransform:"uppercase"}}>Traitements de cette séance</div>
          {items.map((item,i)=>(
            <div key={i} style={S.itemCard}>
              {item.type==="continuer"?(
                <div style={{display:"flex",flexDirection:"column",gap:8}}>
                  <div style={{display:"flex",gap:8,alignItems:"center"}}>
                    <span style={{...S.pill("#dbeafe","#1e40af"),flexShrink:0}}>🔄 Continuer</span>
                    <select style={{...S.fInput,flex:1}} value={item.ref} onChange={e=>updateItem(i,"ref",e.target.value)}>
                    ...
                    <button style={S.btnRedSm} onClick={()=>removeItem(i)}>✕</button>
                  </div>
                  {item.ref&&(()=>{
                    const {prixTotal,totalVerse,reste}=getTreatSummary(actes,item.ref);
                    return <div style={S.infoChip}>Plan: <b>{fmt(prixTotal)}</b> | Payé: <b style={{color:"#16a34a"}}>{fmt(totalVerse)}</b> | <b style={{color:"#ef4444"}}>Reste: {fmt(reste)}</b></div>;
                  })()}
                  <div style={{display:"flex",alignItems:"center",gap:8}}>
                    <label style={{...S.fLabel,whiteSpace:"nowrap"}}>Remise (DA)</label>
                    <input style={{...S.fInput,width:110}} type="number" min={0} value={item.remise} onChange={e=>updateItem(i,"remise",Number(e.target.value))} placeholder="0"/>
                  </div>
                </div>
              ):(
                <div style={{display:"flex",flexDirection:"column",gap:8}}>
                  <div style={{display:"flex",gap:8,alignItems:"center",flexWrap:"wrap"}}>
                    <span style={{...S.pill("#ede9fe","#6d28d9"),flexShrink:0}}>✨ Nouveau</span>
                    ...
                    <div style={{display:"flex",flexDirection:"column",alignItems:"center",gap:1}}>
                      ...
                      <input style={{...S.fInput,width:48,textAlign:"center"}} type="number" min={1} value={item.quantite} onChange={e=>updateItem(i,"quantite",Math.max(1,parseInt(e.target.value)||1))}/>
                    </div>
                    {TARIFS[item.typeActe]?.length>1?(
                      <select style={{...S.fInput,width:110}} value={item.prixUnitaire} onChange={e=>updateItem(i,"prixUnitaire",Number(e.target.value))}>
                        {TARIFS[item.typeActe].map(p=><option key={p} value={p}>{fmt(p)}</option>)}
                      </select>
                    ):(
                      <input style={{...S.fInput,width:110}} type="number" value={item.prixUnitaire} onChange={e=>updateItem(i,"prixUnitaire",Number(e.target.value))}/>
                    )}
      ...
      {patientId&&items.length>0&&(
        <>
          <div style={S.formRow}>
            <div style={S.fGroup}><label style={S.fLabel}>Étape / Nom de la séance</label><input style={S.fInput} value={etape} onChange={e=>setEtape(e.target.value)} placeholder="ex: Préparation, Mise en forme…"/></div>
            <div style={S.fGroup}><label style={S.fLabel}>🔬 Diagnostic</label><input style={S.fInput} value={diagnostic} onChange={e=>setDiagnostic(e.target.value)} placeholder="ex: Pulpite, Abcès…"/></div>
          </div>
          ...
          <div style={S.fGroup}>
            <label style={S.fLabel}>💳 Versement total ce jour (DA)</label>
            <input style={{...S.fInput,maxWidth:200}} type="number" min={0} value={versement} onChange={e=>setVersement(e.target.value)} placeholder="0"/>
          </div>
        </>
      )}
...
function EditActeModal({acte,onClose,onSave}){
  const [f,setF]=useState({
    ...
    prixUnitaire:acte.prixUnitaire||0,prixTotal:acte.prixTotal||acte.prix||0,
    remise:acte.remise||0,etape:acte.etape||"",diagnostic:acte.diagnostic||"",
  ...
  return(
    <Modal title="Modifier la séance" onClose={onClose}>
      ...
      <div style={S.formRow}>
        ...
        <div style={S.fGroup}><label style={S.fLabel}>Remise (DA)</label><input style={S.fInput} type="number" min={0} value={f.remise} onChange={e=>set("remise",Number(e.target.value))}/></div>
      </div>
...
// ══════════════════════════════════════════════════════════════════════════════
// EDIT VERSEMENT MODAL (rapide depuis historique)
// ══════════════════════════════════════════════════════════════════════════════
function EditVersementModal({acte,onClose,onSave}){
  ...
  return(
    <Modal title="Modifier le versement" onClose={onClose}>
      <div style={{...S.infoChip,marginBottom:12}}>
        Séance S{acte.seanceNum||"?"} — {acte.date} — {acte.typeActe} {acte.dents?`Dent ${acte.dents}`:""}
      </div>
      <div style={S.fGroup}>
        <label style={S.fLabel}>💳 Montant versé (DA)</label>
        <input style={{...S.fInput,fontSize:18,fontWeight:700,textAlign:"center"}} type="number" min={0} value={v} onChange={e=>setV(Number(e.target.value))}/>
      </div>
      <div style={S.mAct}>
        <button style={S.btnGray} onClick={onClose}>Annuler</button>
...
const S={
  ...
  recentRow:{display:"grid",gridTemplateColumns:"100px 1fr 1fr 90px",gap:10,padding:"9px 14px",background:"#fff",borderRadius:8,marginBottom:5,fontSize:13,alignItems:"center"},
  ...
  finItem:{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"6px 0",borderBottom:"1px solid rgba(0,0,0,.05)",fontSize:14},
  ...
  sessHdr:{display:"grid",gridTemplateColumns:"36px 82px 1fr 1fr 78px 30px",gap:6,padding:"3px 7px",fontSize:10,color:"#94a3b8",fontWeight:700,textTransform:"uppercase"},
  sessRow:{display:"grid",gridTemplateColumns:"36px 82px 1fr 1fr 78px 30px",gap:6,padding:"6px 7px",background:"#fff",borderRadius:6,fontSize:12,alignItems:"center"},
  ...
  itemCard:{background:"#f8fafc",border:"1px solid #e2e8f0",borderRadius:10,padding:"12px 14px",marginBottom:8},
  ...
  modeBtn:{flex:1,padding:"9px 10px",border:"2px solid #e2e8f0",borderRadius:8,background:"#fff",cursor:"pointer",fontSize:13,fontWeight:500,minWidth:150},
